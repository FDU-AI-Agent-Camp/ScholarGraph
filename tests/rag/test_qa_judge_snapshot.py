...
async def test_invoke_judge_structured_output_uses_with_structured_output() -> None:
    ...
    structured_runnable.ainvoke = AsyncMock(return_value=snapshot)
    chat = MagicMock()
...
@pytest.mark.asyncio
async def test_invoke_qa_judge_live_path_uses_structured_output_snapshot(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _return_snapshot(_client: object, _messages: object, **_kwargs: object) -> JudgeSchema:
        return snapshot

    with patch("backend.rag.qa_judge.invoke_judge_structured_output", side_effect=_return_snapshot):
...
def test_snapshot_dual_track_evaluation_is_repeatable() -> None:
    ...
    assert evaluation["faithfulness"]["semantic_alignment"] == 0.92
    assert evaluation["judge"]["reasoning"].startswith("Snapshot:")
