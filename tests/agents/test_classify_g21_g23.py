...
from backend.agents.classifier_llm import _invoke_structured, classify_with_llm
...
@pytest.mark.asyncio
async def test_g21_invoke_structured_model_validates_dict_payload(live_classify_env: None) -> None:
    ...
    structured_runnable = MagicMock()
    structured_runnable.ainvoke = AsyncMock(return_value=payload)
    chat = MagicMock()
    chat.with_structured_output.return_value = structured_runnable
    client = LlmClient()
    client._chat = chat
