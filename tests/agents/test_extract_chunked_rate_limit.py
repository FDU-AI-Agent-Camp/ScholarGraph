...
class TestExtractChunkedRateLimit:
    async def test_acquires_rate_limit_before_each_llm_call(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            patch(
                ...
                new=AsyncMock(return_value=_node_list(1)),
            ),
            patch(
                "backend.agents.extract_chunked.build_edges_with_llm",
                new=AsyncMock(return_value=_edge_list(1)),
            ),
            patch(
                "backend.agents.extract_chunked.merge_graphs",
                new=MagicMock(return_value=_graph_result(paper_id)),
            ),
        ):
    ...
    async def test_does_not_hard_cap_at_old_max_chunks(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with (
            ...
            patch(
                "backend.agents.extract_chunked.merge_graphs",
                new=MagicMock(return_value=_graph_result(paper_id, node_count=25)),
            ),
        ):
    ...
    async def test_chunk_count_exceeds_safety_limit_truncates(
        ...
        async def tracked_extract_nodes(*args, **kwargs) -> ExtractedNodeList:
        ...
        with (
            patch(
                "backend.agents.extract_chunked.extract_nodes_with_llm",
                new=AsyncMock(side_effect=tracked_extract_nodes),
            ),
            patch(
                "backend.agents.extract_chunked.build_edges_with_llm",
                new=AsyncMock(return_value=_edge_list(1)),
            ...
            patch(
                "backend.agents.extract_chunked.merge_graphs",
                new=MagicMock(return_value=_graph_result(paper_id, node_count=10)),
            ),
        ):
    ...
    async def test_semaphore_limits_concurrent_llm_calls(
        ...
        with (
            patch(
                "backend.agents.extract_chunked.extract_nodes_with_llm",
                new=AsyncMock(side_effect=slow_extract_nodes),
            ),
            patch(
                "backend.agents.extract_chunked.build_edges_with_llm",
                new=AsyncMock(side_effect=slow_build_edges),
            ),
            patch(
                "backend.agents.extract_chunked.merge_graphs",
                new=MagicMock(return_value=_graph_result(paper_id, node_count=10)),
    ...
    async def test_empty_node_lists_raises_value_error(
        ...
        with (
            patch(
                "backend.agents.extract_chunked.extract_nodes_with_llm",
                new=AsyncMock(return_value=empty_nodes),
            ),
            patch(
                "backend.agents.extract_chunked.build_edges_with_llm",
                new=AsyncMock(return_value=_edge_list(0)),
            ),
