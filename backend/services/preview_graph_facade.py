...
class PreviewGraphFacade:
    ...
    async def save(self, paper_id: str, graph: UnifiedPaperGraph) -> None:
        await self._pipeline_repo.save_preview_graph(paper_id, graph)

    async def get(self, paper_id: str) -> UnifiedPaperGraph | None:
        return await self._pipeline_repo.get_preview_graph(paper_id)

    async def clear(self, paper_id: str) -> None:
        await self._pipeline_repo.clear_preview_graph(paper_id)

    async def mark_available(self, paper_id: str) -> None:
        await self._core_service.mark_preview_available(paper_id)

    async def is_available(self, paper_id: str) -> bool:
