"""Heuristic graph extraction — fallback only when LLM structured extract fails."""
...
def paper_id_for_text(text: str) -> str:
...
def build_hss_graph(full_text: str, title: str) -> UnifiedPaperGraph:
    ...
    return UnifiedPaperGraph(
        paper_id=paper_id_for_text(full_text),
        title=title,
        paradigm=Paradigm.HSS,
        nodes=nodes,
        edges=edges,
        summary="启发式 fallback：HSS 论证图谱（核心论点、分论点、理论视角、研究对象与学术谱系）。",
    )


def build_stem_graph(full_text: str, title: str) -> UnifiedPaperGraph:
    ...
    return UnifiedPaperGraph(
        ...
        nodes=nodes,
        edges=edges,
        summary="启发式 fallback：STEM 实验验证图谱（问题、方法、数据集、指标、基线、声称与证据）。",
    )


def build_heuristic_graph(full_text: str, paradigm: Paradigm, *, title: str | None = None) -> UnifiedPaperGraph:
    """Build a deterministic fallback graph for the requested paradigm."""
    resolved_title = title or extract_title(full_text)
