...
- **状态写入封装（已收拢）**：RAG handler / indexing watchdog / status guard / re-extract **不得**直触 `PaperService._pipeline_repo`；经 `PaperPipelineOpsService` 公开方法（`promote_paper_to_terminal_status` / `promote_stuck_indexing_paper[_sync]` / `touch_indexing_heartbeat` / `initialize_pipeline_snapshot` / `heal_pipeline_snapshot` 等）。sync 扫库物理写在 `backend/repositories/pipeline_sync.py`，由 `PaperPipelineOpsService.promote_stuck_indexing_paper_sync` 编排 + 发 `RagIndexed`。  
  - **状态机闸**：`promote_paper_to_terminal_status` 仅允许 `INDEXING→ready*`；其它来源抛 `InvalidStateTransitionError`。底层 `_save_pipeline_snapshot` 为私有落库核；上传走 `initialize_pipeline_snapshot`（硬约束 PENDING），契约/双表修复走 `heal_pipeline_snapshot`（必填 `reason` + WARNING 审计）。LoD AST 只锁 `_pipeline_repo` 穿透，不是完整状态机防火墙。  
